...
class ClientThread(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            if len(data) < HDR or (
                data[0] != ord("D") and data[0] != ord("E")
            ):  # Esto es un error, no entendí
                continue
            ...
            if from_seq(data[1], data[2]) == cnt:
                ...
                if data[0] == ord("E"):  # EOF
                    # Envio ACK del EOF
                    conn.send(
                        b"A" + bytes([data[1], data[2]]) + str(tbytes).encode("UTF-8")
                    )
                ...
                print(
                    f"Recibi paquete malo: {from_seq(data[1],data[2])} expecting: {cnt}"
                )
