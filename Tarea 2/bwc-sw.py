...
tries = 0
init_time = time.time()
while tries < MAX_TRIES and time.time() - init_time < MAX_TIME:
    # Anuncio Conexión
...
while time.time() - init_time < MAX_TIME:
    ...
    while tries < MAX_TRIES:
        ...
        if (
            not ack or ack[0] != ord("A") or from_seq(ack[1], ack[2]) != cnt
        ):  # incluye timeout
            print("falla ack")
        else:
...
while tries < MAX_TRIES and time.time() - init_time < MAX_TIME:
    ...
    if (
        not ack
        or ack[0] != ord("A")
        or from_seq(ack[1], ack[2]) != cnt
        or len(ack) == HDR
    ):  # Este ACK NO debe ser sólo Header
        tries = tries + 1
        print("retry eof")
    else:
        break
